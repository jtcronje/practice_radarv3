...
const DoctorAnalysisPage = () => {
  // State management - only keep essential states
  ...
  // Load data on component mount - optimize loading
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        // Create a simplified promise-based parser
        ...
        const [doctors, procedures, billing, patients, hospitals] = await Promise.all([
          parseCSV('doctors.csv'),
          parseCSV('procedures.csv'),
          parseCSV('billing.csv'),
          parseCSV('patients.csv'),
          parseCSV('hospitals.csv')
        ]);
